...
export const useContactHandlers = () => {
  ...
  const onContactDeleteEvent = (connection: signalR.HubConnection) => {
    connection.on(
  ...
  const onContactUpdatedEvent = (connection: signalR.HubConnection) => {
    connection.on(
  ...
  const onContactRequestReceivedEvent = async (
    connection: signalR.HubConnection
  ) => {
    connection.on(
      SignalRHubMethods.ConnectionRequestReceived,
      async (data: IContactRequest) => {
